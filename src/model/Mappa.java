...
public class Mappa {
	...
	// Passo al costruttore della mappa il numero di livelli che dovrà avere
	public Mappa(int d) {
		this.dim = d;
		// Il metodo Utilities.endLiv(dim) fornisce l'id dell'ultimo esagono del
		// livello dim. Quindi se dim è il numero di livelli della mappa, il
		// numero di esagoni della mappa è Utilities.endLiv(dim)+1 (Considerando
			...
			for (int s = 1; s <= 6; s++) {
				...
				for (int p = 0; p < l; p++) {
					...
					if (p != 0) {
						...
						// Se sto considerando un esagono che non ha nè
						// posizione 0 nè posizione massima in un settore
						else {
